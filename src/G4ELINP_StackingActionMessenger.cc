...
#include "G4ELINP_StackingActionMessenger.hh"
...
G4ELINP_StackingActionMessenger::G4ELINP_StackingActionMessenger(G4ELINP_StackingAction* stack)
:fStackAction(stack)
{
...
G4ELINP_StackingActionMessenger::~G4ELINP_StackingActionMessenger()
{
  	delete fKillCmd;
  	delete fKCmd;
}
...
void G4ELINP_StackingActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{     
  	if(command == fKillCmd) {fStackAction->SetKillStatus(fKillCmd->GetNewBoolValue(newValue));} 	
  	else if(command == fKCmd) {fStackAction->SetKill(newValue);}
}
